...
const winCheck = (row, column) => {
    if (checkAdjacentRowValues(row) || checkAdjacentColumnValues(column) || checkAdjacentDiagonalValues(row, column)) {
      // Call endGame function with the current player as the winner
...
const setPiece = (startCount, colValue) => {
  ...
  if (initialMatrix[startCount][colValue] != 0) {
    ...
    if (winCheck(startCount, colValue)) {
      message.innerHTML = `GAME OVER <br>Player<span> ${currentPlayer}</span> wins!!`;
      startScreen.classList.remove("hide");
      return false;
    }
  }
...
const endGame=(winner) => {
    let winnerText = winner ? `Player ${winner} wins!` : "It's a draw!";
...
document.getElementById('homeButton').addEventListener('click', () =>  {
    window.location.reload(true); // Reload the page. Or use `window.location.href` to redirect.
});
